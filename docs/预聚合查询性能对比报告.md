...
| 连接 | 默认与 `utils/setup.py views` 一致：`111.229.81.45:3306`，用户 `agentic_bi` |
...
对**每一种分析口径**（与 `utils/schema.sql` `@section views` 中视图定义一致），执行三类 SQL 并测量**端到端**耗时（执行 + 拉取全部结果行，避免仅测 “首包” 时间）：
...
- 视图定义变更后：先执行 `python utils/setup.py views`，再运行本基准脚本。
