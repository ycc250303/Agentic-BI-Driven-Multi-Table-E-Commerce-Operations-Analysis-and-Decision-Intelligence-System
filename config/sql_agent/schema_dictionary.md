...
- 比率类**排名**（`aggregation` 为 topN）必须 `HAVING` 过滤分组样本量（默认已送达 / 已评订单数 >= 20）。列出全部分组的比率对比不要 HAVING。
- 未指定返回条数时：问「哪个 / 哪种」取 Top 1；问「哪些」取 Top 10；卖家等大基数实体上的比率排名取 Top 20（与上条样本量门槛一致）。
- 一条查询一个 grain：全表标量（总体均值、相关系数）不要和 `GROUP BY` 分布 / 分桶写在同一 `SELECT`。
- 重量分桶默认（克，左闭右开）：`[0,500)`、`[500,1000)`、`[1000,2000)`、`[2000,5000)`、`5000+`；桶名写成 `0-499g` / `500-999g` / `1000-1999g` / `2000-4999g` / `5000g+`。
- 准时 vs 超时的分箱标签用 `on_time` / `late`（不要用 delayed 或中文）。
- 差评率按订单：已评订单为分母、`review_score<=2` 的订单为分子，均为 `COUNT(DISTINCT order_id)`。
- 客户数 / GMV / 复购等销售口径订单状态与视图一致：`delivered, shipped, created, approved, processing, invoiced`。
- 运费占比、复购、重量-运费关系不在预聚合销售视图字段中，必须回退 `order_items`（及 `orders` / `customers` / `products`）。
- 延迟率 = 延迟订单 / 已送达订单（订单须 `order_status='delivered'` 且签收日、预计送达日非空）；准时率同口径按「签收日 ≤ 预计送达日」计算。
- **平台整体**准时率 / 延迟率必须在 `orders` 上按订单重算；不能对 `mv_delivery_perf.on_time_rate` 再做无权重或按 `delayed_orders` 加权的 AVG/SUM 冒充全平台。
